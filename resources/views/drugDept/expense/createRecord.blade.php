...
<body class="h-full">
    <div class="container p-4 mx-auto">
        ...
        <div class="bg-white shadow-md rounded-lg p-2 sm:w-full sm:max-w-lg mx-auto">
            <form class="space-y-6" action="{{ route('expenseRecord.store') }}" method="POST">
                ...
                <div id="medicineFields" class="space-y-4">
    ...
    let newField = `
        <div class="flex items-center p-4 border border-gray-200 rounded-lg bg-gray-50 space-x-4 mb-4" id="medicineField_${medicineIndex}">
            <div class="relative flex-grow">
                ...
                <input type="text" id="medicine_search_${medicineIndex}" name="medicine_search[]" class="w-full border rounded-md px-4 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" oninput="searchMedicine(${medicineIndex})" placeholder="Type to search...">
...
function searchMedicine(index) {
    ...
    if (filteredMedicines.length > 0) {
        filteredMedicines.forEach(medicine => {
            ...
            resultsHTML += `<div class="p-2 hover:bg-gray-100 cursor-pointer" onclick="selectMedicine(${index}, '${medicine.id}', '${medicine.name}', '${generic ? generic.generic_name : ''}')">${medicine.name} (${generic ? generic.generic_name : ''})</div>`;
        });
    } else {
    ...
    resultContainer.classList.remove('hidden');
        ...
        function selectMedicine(index, id, name, genericName) {
